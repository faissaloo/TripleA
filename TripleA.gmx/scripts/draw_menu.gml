...
for (i=0;i<ds_list_size(item_list);i+=1) //Go through the list of menu items
{
    ...
    if (mouse_x>xx) and (mouse_x<xx+width+hpadding) and (mouse_y<yy+(height/2)*(i+1)+vpadding) and (mouse_y>yy+vpadding) and mouse_check_button_released(mb) //Adding hpadding so it spans the whole cell
    {
